...
.PHONY: venv postgres server stop bootstrap warm-cache warm-sector-cache precompute-app-snapshots price-snapshots backfill-snapshots quality-gates factor-policy policy-backtest remediation-plan research-quality research-quality-docker research-health data-quality ops-health workspace-audit secret-audit ci-local regen-mobile-clients ios-build ios-test android-build mobile-smoke ios-simulator-qa account-watchlist-smoke staging-url staging-readiness staging-smoke staging-ops-health staging-status idle-staging stop-staging start-staging restart-staging delete-staging create-staging-resources sync-staging-secrets deploy-staging-local bootstrap-staging-data staging-research-quality android-device-qa linux-deploy linux-price-refresh linux-install-systemd print-ops-schedule install-ops-schedule uninstall-ops-schedule ops-schedule-status print-price-schedule install-price-schedule uninstall-price-schedule price-schedule-status print-staging-ops-schedule install-staging-ops-schedule print-research-schedule install-research-schedule uninstall-research-schedule research-schedule-status configure-app-api app-smoke user-flow qa api dag-dry-run test
...
ci-local:
	...
	$(PYTHON) -m py_compile main_engine.py main_dag.py api/server.py api/auth.py api/auth_store.py api/runtime_state.py api/contracts/mobile_v1.py api/routers/calendar.py api/routers/etfs.py api/routers/market.py api/routers/news.py api/routers/ops.py api/routers/portfolio.py api/routers/risk.py api/routers/search.py api/routers/sectors.py api/routers/stock.py api/services/calendar_api.py api/services/etf_api.py api/services/etf_insights.py api/services/market_api.py api/services/news_api.py api/services/news_impact.py api/services/ops_api.py api/services/portfolio_api.py api/services/risk_api.py api/services/search_api.py api/services/sector_api.py api/services/stock_detail.py pipeline/02_macro_regime.py tools/audit_workspace.py tools/check_ci_workflows.py tools/check_data_quality.py tools/check_ops_health.py tools/check_no_local_artifacts.py tools/check_research_quality_health.py tools/check_staging_status.py tools/configure_app_api.py tools/install_ops_health_launchd.py tools/install_price_snapshots_launchd.py tools/install_research_quality_launchd.py tools/qa_android_device.py tools/run_research_quality.py tools/sync_price_snapshots.py tools/precompute_app_snapshots.py tools/warm_sector_cache.py GitHub/my-quant-dashboard/app.py GitHub/my-quant-dashboard/data_loader.py quantbridge/config.py quantbridge/price_snapshots.py quantbridge/quality.py quantbridge/schemas.py quantbridge/storage/postgres.py quantbridge/storage/repository.py
	$(PYTHON) tools/check_no_local_artifacts.py
	$(PYTHON) -m unittest test_contracts.py test_smallcap_scoring.py test_data_quality.py test_api_auth.py test_api_ops.py test_config.py test_pipeline_imports.py
	...
	$(PYTHON) -c "import plistlib,sys; [print(f'{p}: OK') if (d:=plistlib.load(open(p,'rb'))).get('Label') and d.get('ProgramArguments') else (_ for _ in ()).throw(SystemExit(f'{p}: invalid plist')) for p in sys.argv[1:]]" /tmp/quantbridge-ops-health.plist /tmp/quantbridge-research-quality.plist
...
mobile-smoke:
	$(PYTHON) tools/mobile_smoke_qa.py
