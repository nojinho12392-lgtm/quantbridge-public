...
class QuantApiServiceTest {
    ...
    fun quantApiServiceSerializesAuthAndWatchlistBodies() = runBlocking {
        ...
        service.signup(AuthRequest(email = "a@b.com", password = "password123", displayName = "Jinho"))
        assertEquals("/auth/signup", capture.lastPath)
        assertTrue(capture.lastBody.contains("\"display_name\":\"Jinho\""))
                ...
                path == "/auth/login" || path == "/auth/signup" -> """
                    {
                      "access_token": "token",
                      "token_type": "bearer",
                      "user": {"id": "user-1", "email": "a@b.com", "display_name": "Jinho"}
                    }
                """.trimIndent()
                else -> """{"ok":true}"""
            }
