...
class OpsViewModel @Inject constructor(
    ...
    var mlBlendReport by mutableStateOf<MLBlendReport?>(null)
        private set
    var policyAdjustedRankings by mutableStateOf<List<PolicyAdjustedRanking>>(emptyList())
        private set
    ...
    fun hasData(): Boolean {
        return researchQuality != null || mlBlendReport != null || policyAdjustedRankings.isNotEmpty() || opsHealth != null
    }
    ...
    private fun applyResult(result: OpsRepositoryResult) {
        ...
        if (result.policyAdjustedRankings.isNotEmpty()) {
            policyAdjustedRankings = result.policyAdjustedRankings
        }
